...
if (window.AudioWorklet !== undefined) {
    class PinkTromboneNode extends AudioWorkletNode {
        ...
        // _isEnabled is flipped synchronously, before the processor round-trip:
        // it is what newConstriction() uses to pick a free slot, so waiting for
        // the worklet's reply would hand the same slot to every constriction
        // created in one synchronous block.
