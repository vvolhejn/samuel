  ...
  _processConstrictions(constrictions, parameterSamples) {
    for (let index = 0; index < constrictions.length; index++) {
      ...
      if (constriction.index >= 2 && constriction.index <= this.length && constriction.diameter > 0) {
        ...
        // A constriction at the lip end (e.g. a fixed lip constriction at
        // index length - 1) would inject past the last cell and be silently
        // dropped (out-of-bounds typed-array writes are no-ops); clamp so
        // the noise lands in the lip cell instead.
        const lowerNoiseIndex = Math.min(lowerIndex + 1, this.length - 1);
        const upperNoiseIndex = Math.min(upperIndex + 1, this.length - 1);

        this.right[lowerNoiseIndex] += lowerNoise;
        this.right[upperNoiseIndex] += upperNoise;

        this.left[lowerNoiseIndex] += lowerNoise;
        this.left[upperNoiseIndex] += upperNoise;
      }
    }
  }
